...
async function generateDailyRecoveryVerses(): Promise<void> {
  ...
  try {
    ...
    // NOTE:
    // - The recovery system is consumed per-user (usage tracking is user-scoped elsewhere).
    // - What we guarantee here is a DAILY pool/category with enough verses for the day.
    // - We intentionally allow the same reference to appear across different days/categories
    //   to ensure users always have verses available, even if the upstream source repeats.
    const targetPerDay = 50;

    const allVerses = await storage.getAllBibleVerses();
    const todaysVerses = allVerses.filter(v => v.category === todayCategory);

    if (todaysVerses.length >= targetPerDay) {
      console.log(`[Recovery Verses Scheduler] Already have ${todaysVerses.length}/${targetPerDay} verses for today (${todayCategory})`);
      return;
    }
    ...
    // Deduplicate only within today's category (not globally)
    const todayReferences = new Set(todaysVerses.map(v => v.reference));

    let addedCount = 0;
    let attempts = 0;

    // Try multiple batches because providers may return fewer verses than requested or repeated content.
    while (todaysVerses.length + addedCount < targetPerDay && attempts < 5) {
      attempts++;

      const remaining = targetPerDay - (todaysVerses.length + addedCount);
      const batchSize = Math.min(remaining, 50); // ask up to 50 per call

      const generatedVerses = await generateRecoveryVersesWithAI(batchSize);
      ...
      for (const verse of generatedVerses) {
        // Avoid duplicates within the same day/category
        if (todayReferences.has(verse.reference)) {
          console.log(`[Recovery Verses Scheduler] Skipping duplicate for today: ${verse.reference}`);
          continue;
        }
        ...
        try {
          ...
          todayReferences.add(verse.reference);
          addedCount++;
        } catch (err: any) {
          // If the DB has uniqueness constraints, don't fail the whole job.
          console.warn(`[Recovery Verses Scheduler] Failed to insert verse ${verse.reference}:`, err?.message || err);
        }

        if (todaysVerses.length + addedCount >= targetPerDay) break;
      }
    }

    console.log(`[Recovery Verses Scheduler] Added ${addedCount} verse(s). Total for ${todayCategory}: ${todaysVerses.length + addedCount}/${targetPerDay}`);
  } catch (error) {
...
/**
 * IMPORTANT ARCHITECTURE NOTES (to avoid impacting other subsystems):
 * - This pipeline is fully idempotent using the scheduler_reminders table (storage.hasSentSchedulerReminder / markSchedulerReminderSent).
 * - Puppeteer is ONLY invoked after the Daily Verse post for TODAY exists AND has a generated reflection.
 * - We keep the existing cron entries as fallback, but publication is guarded to avoid double-posting.
 */

function toSaoPauloDateKey(date: Date | string): string {
...
async function waitForDailyVersePostReady(todayKey: string, timeoutMs = 8 * 60 * 1000, pollMs = 10_000): Promise<any | null> {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (post) {
      const postKey = toSaoPauloDateKey(post.publishedAt || post.createdAt || new Date());

      const hasVerse = !!post.verse && !!post.reference;
      const hasReflection = typeof post.reflection === 'string' && post.reflection.trim().length > 0;

      if (postKey === todayKey && hasVerse && hasReflection) {
        return post;
      }
    }
...
function getInstagramStoryReminderKey(kind: 'verse' | 'reflection', todayKey: string): string {
  return `ig_story_published:${kind}:${todayKey}`;
}

async function isInstagramStoryPublished(kind: 'verse' | 'reflection', todayKey: string): Promise<boolean> {
  return storage.hasSentSchedulerReminder(getInstagramStoryReminderKey(kind, todayKey));
}

async function markInstagramStoryPublished(kind: 'verse' | 'reflection', todayKey: string): Promise<void> {
  await storage.markSchedulerReminderSent(getInstagramStoryReminderKey(kind, todayKey), 'instagram_story');
}

// Wrapper to keep this file working even if story-image-generator has a slightly different function signature.
// Extra params are harmless in JS; they will be ignored if not used.
const uploadToR2 = async (buffer: Buffer, folder: string, contentType: string, filename: string): Promise<string> => {
  return (uploadStoryImageToR2 as any)(buffer, folder, contentType, filename);
};
...
export async function generateAndSaveStoryImages() {
  ...
  try {
    ...
    if (!isInstagramConfigured() || !isInstagramPublishingConfigured()) {
      console.log('[Instagram Stories] Instagram not configured. Skipping story pipeline.');
      return;
    }

    // 2. Garante que o versículo do dia já existe no banco antes de gerar imagem
    const readyPost = await waitForDailyVersePostReady(todayKey);
    if (!readyPost) {
      console.error(`[Instagram Stories] Daily verse/reflection not ready for today (${todayKey}). Skipping generation.`);
      return;
    }

    // --- GERAÇÃO DA IMAGEM DO VERSÍCULO ---
    // FIX: Passando todayKey (Data) como primeiro argumento, pois o erro indicava que 'verse' estava indo para um campo de data
    let existingVerseImage = null;
    try {
        existingVerseImage = await storage.getDailyStoryImage(todayKey, 'verse');
    } catch (e) {
        // Tenta a ordem inversa caso o método no storage esteja definido diferente, mas prioriza Data Primeiro
        try { existingVerseImage = await storage.getDailyStoryImage('verse', todayKey); } catch (e2) {}
    }

    if (!existingVerseImage) {
      console.log('[Instagram Stories] Generating verse image (Puppeteer)...');
      
      // Gera a imagem usando o Puppeteer
      const verseBuffer = await generateVerseShareImage(readyPost.verse, readyPost.reference, 'story');

      // Upload para o R2
      const filename = `verse-story-${todayKey}.png`;
      const verseR2Key = await uploadToR2(verseBuffer, 'stories', 'image/png', filename);
      ...
      await storage.saveDailyStoryImage({
        ...
        r2Key: verseR2Key,
        verse: readyPost.verse, // Salva o texto para referência
        reference: readyPost.reference
      });
    ...
    // --- GERAÇÃO DA IMAGEM DA REFLEXÃO ---
    // FIX: Mesma correção da ordem dos argumentos
    let existingReflectionImage = null;
    try {
        existingReflectionImage = await storage.getDailyStoryImage(todayKey, 'reflection');
    } catch (e) {
        try { existingReflectionImage = await storage.getDailyStoryImage('reflection', todayKey); } catch (e2) {}
    }

    if (!existingReflectionImage && readyPost.reflection) {
      console.log('[Instagram Stories] Generating reflection image (Puppeteer)...');
      
      const reflectionBuffer = await generateReflectionShareImage(
        readyPost.reflection, 
        readyPost.reflectionTitle || "Reflexão do Dia",
        readyPost.verse,
        readyPost.reference
      );

      const filename = `reflection-story-${todayKey}.png`;
      const reflectionR2Key = await uploadToR2(reflectionBuffer, 'stories', 'image/png', filename);
      ...
      console.log('[Instagram Stories] Reflection image already generated/skipped.');
    }

    // Tenta publicar
    const versePublished = await publishVerseStoryToInstagramInternal(todayKey);
    if (versePublished) {
      await publishReflectionStoryToInstagramInternal(todayKey);
    }
...
async function publishVerseStoryToInstagramInternal(todayKey: string): Promise<boolean> {
  console.log(`[Instagram Stories] Attempting to publish verse story for ${todayKey}...`);

  try {
    if (!isInstagramConfigured() || !isInstagramPublishingConfigured()) {
      console.log('[Instagram Stories] Instagram not configured for publishing. Skipping verse story publish.');
      return false;
    }

    if (await isInstagramStoryPublished('verse', todayKey)) {
      console.log('[Instagram Stories] Verse story already published for today - skipping.');
      return true;
    }
    ...
    if (result.success) {
      ...
      await markInstagramStoryPublished('verse', todayKey);
      return true;
    }
...
// 2. Publica o Story do Versículo (Roda às 07:05) - kept as fallback/compatibility
async function publishVerseStoryToInstagram() {
  const todayKey = getTodayDateKey();
  await publishVerseStoryToInstagramInternal(todayKey);
}

async function publishReflectionStoryToInstagramInternal(todayKey: string): Promise<boolean> {
  console.log(`[Instagram Stories] Attempting to publish reflection story for ${todayKey}...`);

  try {
    if (!isInstagramConfigured() || !isInstagramPublishingConfigured()) {
      console.log('[Instagram Stories] Instagram not configured for publishing. Skipping reflection story publish.');
      return false;
    }

    // Enforce ordering: only publish reflection after verse succeeded
    const verseOk = await isInstagramStoryPublished('verse', todayKey);
    if (!verseOk) {
      console.log('[Instagram Stories] Verse story not published yet - skipping reflection publish to preserve ordering.');
      return false;
    }

    if (await isInstagramStoryPublished('reflection', todayKey)) {
      console.log('[Instagram Stories] Reflection story already published for today - skipping.');
      return true;
    }
    ...
    if (result.success) {
      ...
      await markInstagramStoryPublished('reflection', todayKey);
      return true;
    }
...
// 3. Publica o Story da Reflexão (Roda às 07:10) - kept as fallback/compatibility
...
export async function generateAndSaveBirthdayImages() {
  ...
  try {
    ...
    const formatter = new Intl.DateTimeFormat('en-US', { timeZone: 'America/Sao_Paulo', day: 'numeric', month: 'numeric', year: 'numeric' });
    ...
    // Constrói a chave de data consistente
    const todayKey = `${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(currentDay).padStart(2, '0')}`;

    // Busca aniversariantes
    const members = await storage.getMembersByBirthday(currentDay, currentMonth);

    // FIX: Verificação explícita de array vazio para evitar erro de sintaxe SQL
    ...
    for (const member of members) {
      try {
        ...
        const existing = await storage.getBirthdayShareImage(member.id, todayKey);
        if (existing) {
             console.log(`[Instagram Stories] Image already exists for ${firstName}, skipping.`);
             continue;
        }
        ...
        // Gera imagem
        const buffer = await generateBirthdayShareImage(member.fullName, member.photoUrl || undefined);

        const filename = `birthday-${member.id}-${todayKey}.png`;
...
async function publishBirthdayStoriesToInstagram() {
  ...
  const today = new Date();
  const todayKey = toSaoPauloDateKey(today);
  const currentDay = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: 'America/Sao_Paulo', day: 'numeric' }).format(today), 10);
  const currentMonth = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: 'America/Sao_Paulo', month: 'numeric' }).format(today), 10);

  try {
    if (!isInstagramConfigured() || !isInstagramPublishingConfigured()) {
      console.log('[Instagram Stories] Instagram not configured for publishing. Skipping birthday stories publish.');
      return;
    }
    ...
    for (const member of members) {
      try {
        ...
        const savedImage = await storage.getBirthdayShareImage(member.id, todayKey);
        if (!savedImage) {
